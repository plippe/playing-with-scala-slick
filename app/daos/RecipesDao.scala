...
class RecipesDaoSlick @Inject()(protected val dbConfigProvider: DatabaseConfigProvider) extends RecipesDao with HasDatabaseConfigProvider[JdbcProfile] {
  ...
  val void = DBIOAction.successful(())
  ...
  def insert(recipe: models.Recipe): Future[Unit] = db.run((table += recipe).andThen(void))
  def update(recipe: models.Recipe): Future[Unit] = db.run(table.filter(_.id === recipe.id).update(recipe).andThen(void))
  def delete(recipe: models.Recipe): Future[Unit] = db.run(table.filter(_.id === recipe.id).delete.andThen(void))
}
